...
**Build N — next frontiers (open).** **R-N47 (next).** The pump+crank tour is discovered, realized, shown
...
a 17–35° crank node but ≥4.3 km/s from capture; cheap-to-REACH, hot-to-ARRIVE), and the Mars CRANK realized
(R-N46: it transfers but INVERTED — the high-v∞ arrival chains 8 steps to 68%/23.8° while the low-v∞ one stalls
at 34%; a sustained crank needs many small re-closing steps, NON-monotone in δmax; Mars is a crank node for HOT
arrivals). Open work, in rough priority:
(a) sweep more Mars arrival epochs to map the crank-realization-vs-v∞ curve (is the δmax-inversion monotone?);
(b) a Mars-capture budget round (what Δv-for-time buys a bound Mars orbit); (c) a discrete crank-branch search
for the R-N42 fraction; (d) a NON-greedy Mars crank (net-neutral setup step, R-N33 style) to test whether it
escapes the t0+200 stall. Other open frontiers: a single accumulating

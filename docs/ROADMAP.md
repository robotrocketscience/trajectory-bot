...
**Build N — next frontiers (open).** **R-N51 (next).** The pump+crank tour is discovered, realized, shown
...
depth-4 pump plus two saturated inert tail legs. Open work, in rough priority:
(a) generalize the differentiable-conditioning axis — map the gradient w.r.t. t0 AND per-flyby turn controls vs
depth (R-N50 did ltof only); (b) a non-saturating deeper pump (mix earth returns past leg 4) to re-measure
conditioning where tail legs still raise v∞; (c) actually RUN multi-step gradient polish on the depth-6 chain
(not R-N50's single +0.5 d probe); (d) lift the ≤8-crank cap — does t0+600 climb past 72%?; (e) a Mars-capture
budget round (what Δv-for-time buys a bound Mars orbit); (f) a discrete crank-branch search for the R-N42
fraction. Other open frontiers: a single accumulating

...
**Build N — next frontiers (open).** **R-N39 (next): consolidate the realized pump+crank tour.** The full
mission shape is now discovered and realized end-to-end against real ephemeris, exactly ballistically: launch
(v∞ 5.95) → 4 pumping flybys (→16.27, R-N37) → 5 cranking flybys (i_rel →27.09° = 97% of the analytic ceiling,
R-N38), every encounter GN-closed, zero DSM, with the re-closure tax quantified. Open work, in rough priority:
(a) EPOCH ROBUSTNESS — is the 16.27/27° tour typical or a lucky epoch? (sweep launch epochs, distribution of
depth / final-v∞ / final-inclination); (b) DIFFERENTIABLE POLISH of the full pump+crank chain (R-N36's outer
loop, sub-day line-searched steps); (c) the outer-planet (Mars) handoff the exact-ballistic chains never afford
(R-N32 H-N32c — likely needs the R-N28 Δv-for-time economy folded back in as BOUNDED DSMs, an honest hybrid);
(d) surface the realized tour + the R-N32→R-N38 discovery arc in the public docs/writeup (a docs round with a
gate, per the public-claims rule). Other open frontiers: a single accumulating

...
**Build N — next frontiers (open).** **R-N49 (next).** The pump+crank tour is discovered, realized, shown
...
(R-N48). Open work, in rough priority:
(a) adopt the dense crank grid as the DEFAULT in mars_crank.py (retire the sparse-grid debt); (b) lift the
≤8-crank cap — does t0+600 climb past 72%?; (c) a Mars-capture budget round (what Δv-for-time buys a bound Mars
orbit); (d) a discrete crank-branch search for the R-N42 fraction. Other open frontiers: a single accumulating

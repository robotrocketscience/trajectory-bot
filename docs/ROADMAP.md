...
**Build N — next frontiers (open).** **R-N35 (next): differentiable multi-planet discovery WITHOUT the Lambert-BVP
singularity — a FORWARD reduced-order model, or a HARD-constrained solver.** R-N32 integrated the multi-planet
pump as a greedy MGA (shallow); R-N33 proved a deep tour EXISTS by enumeration (beam, 6 legs, v∞ 12); R-N34 then
tested the north star — can a DIFFERENTIABLE optimizer DISCOVER that deep tour? — and found NAIVE gradient descent
through the real-ephemeris Lambert BVP is OBSTRUCTED: gradients flow, but the resonant-return legs the depth
requires are Lambert BVP near-singularities (v∞→50-60 km/s, |grad|→1e18) that the optimizer reward-hacks into,
violating flyby closure, even with clip/NaN-skip/penalty-continuation/reward-cap. So the discrete/singular
resonance structure is the wall (consistent with R-N7). The honest DISCOVERY path is therefore: (a) — the north
star, reformulated — differentiable discovery via a FORWARD reduced-order model (the R-N20 Rodrigues-node /
Tisserand-contour formulation, which has NO BVP singularity) lifted to multi-planet, OR a HARD-constrained solver
(flyby v∞-matching as equality constraints, e.g. multiple-shooting / SLSQP) rather than naive gradients through
Lambert; either should LEARN a deep pump-handoff tour (the beam proved one exists to ≥6 legs / v∞ 12), ideally
also cracking the outer-planet (Mars) handoff the greedy/beam chains couldn't afford (R-N32 H-N32c); and (b) fold
in the inclination crank (R-N16/R-N20) so the discovered tour raises inclination as well as v∞. Other open frontiers: a single accumulating

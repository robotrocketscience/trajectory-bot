...
**Build N — next frontiers (open).** **R-N45 (next).** The pump+crank tour is discovered, realized, shown
...
well-phased epochs) — and R-N39's lone pump-failure epoch autopsied and reclassified as a min-v∞
launch-heuristic artifact (R-N44), not a real dead-spot. Open work, in rough priority: (a) a full pumped
Mars-ARRIVAL tour (chain R-N43's DSM leg on and crank inclination at Mars); (b) a discrete crank-branch
search for the R-N42 fraction; (c) re-run R-N39 viability with a max-pump (not min-v∞) launch rule. Other open frontiers: a single accumulating

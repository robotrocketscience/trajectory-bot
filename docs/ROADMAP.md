...
**Build N — next frontiers (open).** **R-N38 (next): consolidate the discovered deep tour.** The north-star arc
is closed AND deep: R-N36 made differentiable discovery work against real ephemeris; R-N37 showed the exact-
ballistic continuation tree is bushy and reaches **6 flybys / v∞ 16.27 (zero DSM)** — the campaign's deepest,
highest real-ephemeris pump — saturating into v∞-neutral resonant returns as δmax collapses. Open work, in
rough priority: (a) EPOCH ROBUSTNESS — is 16.27 typical or a lucky epoch? (sweep launch epochs, distribution of
depth/final-v∞); (b) the INCLINATION CRANK at the saturated endpoint — v∞ 16.27 at Venus ⇒ free-inclination
ceiling ≈ arcsin(16.27/35.0) ≈ 27.7°, and the trailing v∞-neutral resonant returns are exactly where cranking
is free (R-N16/R-N20 folded into the real-ephemeris deep chain); (c) DIFFERENTIABLE POLISH of the 6-flyby
chain (R-N36's outer loop, sub-day line-searched steps); (d) the outer-planet (Mars) handoff the exact-
ballistic chains never afford (R-N32 H-N32c — likely needs the R-N28 Δv-for-time economy folded back in as
BOUNDED DSMs, an honest hybrid). Other open frontiers: a single accumulating

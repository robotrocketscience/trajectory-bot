...
**Build N — next frontiers (open).** **R-N42 (next).** The pump+crank tour is discovered, realized, shown
TYPICAL (R-N39), publicly surfaced (R-N40), and its continuous freedom is now gradient-polished (R-N41:
16.27 → 17.01, the "search chooses / gradients polish" split completed at full depth). Open work, in rough
priority: (a) POLISH THE CRANK — R-N41 polished the pump objective only; the same 2-D line-searched ascent
on the inclination objective at the saturated node is the natural sibling round (does gradient polish buy
inclination past the 97% the coarse crank walk already reached, or is the re-closure tax a hard continuous
wall?); (b) the outer-planet (Mars) handoff the exact-ballistic chains never afford (R-N32 H-N32c — likely needs the

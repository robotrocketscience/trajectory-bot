...
pump-failure mode). Crank availability generic: 62% of saturated nodes have closed inclination-raising returns
...
**Build N — next frontiers (open).** **R-N44 (next).** The pump+crank tour is discovered, realized, shown
...
the discrete basin choice sets) — and the outer-planet (Mars) frontier crossed (R-N43: exact-ballistic
excludes Mars by a PHASING wall, not energy; a bounded ≤1 km/s mid-course DSM affords the handoff at
well-phased epochs). Open work, in rough priority: (a) the pump-failure mode at
t0+1000 (viable launch, no pump — why?); (b) a full pumped Mars-ARRIVAL tour (chain R-N43's DSM leg on and
crank inclination at Mars); (c) a discrete crank-branch search for the R-N42 fraction. Other open frontiers: a single accumulating

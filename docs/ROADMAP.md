...
**Build N — next frontiers (open).** **R-N34 (next, the north star): a DIFFERENTIABLE optimizer that DISCOVERS a
deep multi-planet pump-handoff tour from a naive objective.** R-N25→R-N31 mapped the multi-planet mechanism
analytically; R-N32 INTEGRATED it end-to-end against real ephemeris as a greedy MGA (composes + pumps faster than
the bound, but SHALLOW: 1-2 legs, Venus-Earth-confined); R-N33 then TESTED whether that shallowness is physical —
a non-greedy BEAM search breaks the wall decisively (6 pump legs, v∞ 12.0, by taking net-negative setup legs), so
the depth EXISTS and is reachable and the greedy shallowness is pure myopia. So the mechanism is understood, its
greedy realization quantified, AND the reachable depth proven — the remaining open work is DISCOVERY (not
existence): (a) — the north star — have the DIFFERENTIABLE optimizer LEARN a deep pump-handoff tour from a naive
objective (the beam has already proven such a tour exists to ≥6 legs / v∞ 12; the diff-sim's job is to discover
it, one level past R-N20's single-planet leverage-then-crank discovery), ideally also cracking the outer-planet
(Mars) handoff the greedy/beam chains couldn't afford (R-N32 H-N32c); and (b) fold in the inclination crank

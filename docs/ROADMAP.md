...
**Build N — next frontiers (open).** **R-N37 (next): DEPTH in the hard-constrained real-ephemeris architecture.**
The differentiable-discovery north-star arc is CLOSED: a deep tour EXISTS by enumeration (R-N33 beam, 6 legs,
v∞ 12); NAIVE gradients through the Lambert BVP are OBSTRUCTED (R-N34); a FORWARD closure-by-construction model
RESCUES discovery idealized (R-N35); and R-N36 married them — hard-constrained forward shooting (fgprop Kepler +
Rodrigues node + Levenberg-GN encounter nulling) makes differentiable discovery WORK against REAL ephemeris
(sub-meter closures, +4.9 km/s physical pump, finite end-to-end gradients, gradient polish improves the tour).
What remains is DEPTH and GENERALITY: (a) replace R-N36's greedy basin choice with a BEAM-style search over
GN-closed continuations (target R-N33's 6-leg depth inside the real-ephemeris hard-constrained architecture),
ideally also cracking the outer-planet (Mars) handoff the greedy/beam chains couldn't afford (R-N32 H-N32c);
(b) fold in the inclination crank (R-N16/R-N20) so the discovered tour raises inclination as well as v∞; and
(c) let the outer differentiable loop optimize MORE than the launch (per-leg basin-continuous parameters,
epoch), with sub-day line-searched steps (R-N36 showed day-scale steps cross phasing discontinuities). Other
open frontiers: a single accumulating

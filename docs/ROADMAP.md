...
**Build N — next frontiers (open).** **R-N36 (next): fold REAL ephemeris back into differentiable discovery via a
HARD-constrained solver.** The differentiable-discovery north star is now bracketed: R-N33 proved a deep tour
EXISTS by enumeration (beam, 6 legs, v∞ 12); R-N34 showed NAIVE gradients through the real-ephemeris Lambert BVP
are OBSTRUCTED (the resonant-return legs are BVP near-singularities the optimizer reward-hacks into, violating
closure); R-N35 showed a FORWARD model (v∞ conserved across flybys BY CONSTRUCTION, no BVP, no singularity)
RESCUES differentiable discovery — gradient ascent discovers a PHYSICAL pump (v∞ 3→12.27, 97% of a grid
enumeration, no reward-hack) — pinning R-N34's obstruction to the BVP FORMULATION. So the open work is to marry
the two: (a) — the reformulated north star — fold REAL ephemeris back in while KEEPING the closure-by-construction
property that made R-N35 work: a HARD-constrained solver (flyby v∞-matching as equality constraints, multiple-
shooting / SLSQP) or a forward real-ephemeris propagation with resonance-targeting, LEARNING a deep real-ephemeris
pump-handoff tour (the beam proved one exists to ≥6 legs / v∞ 12), ideally also cracking the outer-planet (Mars)
handoff the greedy/beam chains couldn't afford (R-N32 H-N32c); and (b) fold in the inclination crank
(R-N16/R-N20) so the discovered tour raises inclination as well as v∞. Other open frontiers: a single accumulating

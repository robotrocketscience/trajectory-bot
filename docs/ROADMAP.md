...
**Build N — next frontiers (open).** **R-N50 (next).** The pump+crank tour is discovered, realized, shown
...
(R-N48), and the dense grid adopted as mars_crank.py's default, retiring the sparse-grid debt (R-N49). Open
work, in rough priority:
(a) lift the ≤8-crank cap — does t0+600 climb past 72%?; (b) a Mars-capture budget round (what Δv-for-time buys
a bound Mars orbit); (c) a discrete crank-branch search for the R-N42 fraction; (d) PIVOT off the Mars-crank
thread (pump depth, the differentiable-discovery north star, a fresh direction). Other open frontiers: a single accumulating

...
confirmed physical. **[PARTIALLY CORRECTED by R-N47: R-N46's interpretation "high-v∞ cranks better / low-v∞
stalls" conflated GEOMETRY with MAGNITUDE and used the UNDER-SURFACING standard grid. At FIXED geometry, low-v∞
gives the HIGHER ceiling-fraction (not lower), and low-v∞ does NOT stall — that stall was a cross-geometry +
sparse-grid artifact; R-N46's fractions (34%/68%) were sparse-grid undercounts (dense ≈ 40%/82%). R-N46's CORE
survives: the crank transfers to Mars, is arrival-dependent, exactly ballistic.]** **R-N47 candidates: sweep more
...
search closes **104–176** — it SYSTEMATICALLY UNDERCOUNTS. Re-running with a dense grid (13×32×60/200-GN)
...
**Build N — next frontiers (open).** **R-N48 (next).** The pump+crank tour is discovered, realized, shown
...
(R-N46: it transfers, arrival-dependent, exactly ballistic), and R-N46's own "high-v∞ cranks better" then
CORRECTED (R-N47: a surfacing-grid artifact — the standard crank grid undercounts; with the dense grid, at
FIXED geometry the ceiling-fraction DECREASES with v∞, no low-v∞ stall, geometry-dependent). Open work, in rough
priority:
(a) re-verify R-N46's headline numbers with the dense crank grid + adopt it in mars_crank.py (its verdicts used
the under-surfacing standard grid); (b) lift the ≤8-crank cap — do high-v∞ fractions climb past 82%?;
(c) a Mars-capture budget round (what Δv-for-time buys a bound Mars orbit); (d) a discrete crank-branch search
for the R-N42 fraction. Other open frontiers: a single accumulating

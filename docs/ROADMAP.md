...
**Build N — next frontiers (open).** **R-N43 (next).** The pump+crank tour is discovered, realized, shown
TYPICAL (R-N39), publicly surfaced (R-N40), and its continuous freedom is now gradient-polished — the PUMP
half genuinely (R-N41: 16.27 → 17.01), the CRANK half only through v∞ (R-N42: absolute inclination rises but
the ceiling-fraction is a near-hard continuous wall the discrete basin choice sets). "Search chooses basins,
gradients polish continuous freedom" is now fully characterized: it buys v∞ but not crank fraction. Open
work, in rough priority: (a) the outer-planet (Mars) handoff the exact-ballistic chains never afford (R-N32 H-N32c — likely needs the
R-N28 Δv-for-time economy folded back in as BOUNDED DSMs, an honest hybrid); (d) the pump-failure mode at
t0+1000 (viable launch, no pump — why?). Other open frontiers: a single accumulating
